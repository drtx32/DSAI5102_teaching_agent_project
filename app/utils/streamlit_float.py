...
<script>
    ...
    if (hostIframe) {
        const hostWrapper = hostIframe.parentNode;
        if (hostWrapper) {
    ...
    if include_unstable_primary:
        st.button(
            "", key=f"__streamlit_float_unstable_primary_{str(uuid.uuid4())[:8]}")
...
def float_parent(css=None):
    ...
    st.markdown(new_css + '\n<div class="float flt-' +
                new_id + '"></div>', unsafe_allow_html=True)
...
def float_box(markdown, width="300px", height="300px", top=None, left=None, right=None, bottom=None, background=None, border=None, shadow=None, transition=None, z_index=None, sticky=False, css=None):
    jct_css = "width: " + width + "; height: " + height + \
        ";border-radius: 0.5rem;padding: 1rem;padding-left: 1.3rem;padding-right: 1.3rem;"
    ...
    st.markdown('<div class="floating flt-' + new_id + '">' +
...
def float_dialog(show=False, width=2, background="slategray", transition=2, transition_from="top", transition_to="center", css=""):
    ...
    float_col_b.float(float_css_helper(width="100%", height="100%", left="0",
                      background="rgba(0, 0, 0, 0.4)", css="z-index: 999000;" + background_pos_css))
    float_col_a.float("padding: 2rem;padding-bottom: 0.9rem;border-radius: 0.5rem;left: 50%;z-index: 999900;" +
                      dialog_pos_css + transition_css + css + "transition-property: top, bottom;background-color: " + background + ";")
...
def float_overlay(show=False, z_index="999999", color="#000000", alpha=0.0, blur="1rem", filter=None):
    ...
    if show:
        float_box("", width="100%", height="100%", left="0", top="0", css=float_css_helper(
