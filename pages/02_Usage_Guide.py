...
- 也可通过页面右下角的设置弹窗进行输入（使用：[`app.module.streamlit_settings_dialog.settings_button`](teaching_agent/module/streamlit_settings_dialog.py)）。
查看运行时配置对象：[`app.utils.config.settings`](teaching_agent/utils/config.py)。
""")
...
- 一键并行启动（同时启动 RAG、websearch 和 Streamlit 主页面）：
  - `python main.py`（内部会分别在后台启动 `teaching_agent/mcp/rag/server.py`、`teaching_agent/mcp/websearch/server.py` 以及 Streamlit）
  - 参见启动脚本：[main.py](main.py)

- 单独启动：
  - 启动 RAG MCP：`python teaching_agent/mcp/rag/server.py`（服务监听端口 8882，参考：[`teaching_agent.mcp.rag.server.RAGVectorStore`](teaching_agent/mcp/rag/server.py)）
  - 启动 WebSearch MCP：`python teaching_agent/mcp/websearch/server.py`（端口 8881）
  - 仅启动前端：`streamlit run 首页.py` 或 `streamlit run app.py`

注意：RAG 服务从 `vectordb/faiss` 加载索引（见：[`teaching_agent/mcp/rag/server.py`](teaching_agent/mcp/rag/server.py)），如更新索引可调用 RAG 的 reload 工具或重启服务。
""")

st.header("5) 使用聊天与 Agent（已实现）")
st.markdown("""
- 在 Streamlit 页面启动后，可通过右下角的 Ask AI 按钮（实现：[`teaching_agent.module.streamlit_ask_ai_dialog.ask_ai_button`](teaching_agent/module/streamlit_ask_ai_dialog.py)）打开聊天窗口。
- 后台的 LangGraph Worker 在需要时由前端启动（实现：[`teaching_agent.agent.worker.LangGraphWorker`](teaching_agent/agent/worker.py)），它会调用 MCP 的 RAG 与 websearch 工具进行检索式问答。
- 日志与运行信息记录由：[`teaching_agent.utils.logging_config.logger`](teaching_agent/utils/logging_config.py) 管理。
""")

st.header("6) 常见问题与调试要点")
st.markdown("""
- 无法找到向量库：确认 `vectordb/faiss` 是否存在，或在 RAG 服务启动后使用 `reload_vectorstore` 工具重新加载（参见 `teaching_agent/mcp/rag/server.py`）。
