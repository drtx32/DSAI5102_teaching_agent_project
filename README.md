...
- **RAG 服务**：`app/mcp/rag/server.py`（本地 MCP 服务，管理 FAISS 向量库并提供检索接口）。
- **模型适配**：`app/llm/llm_provider.py`（封装多厂商 LLM 调用）。
- **UI 组件**：右下角设置与 Ask AI（`app/module/streamlit_settings_dialog.py`、`app/module/streamlit_ask_ai_dialog.py`）。
...
- `app/mcp/rag/server.py`：RAG 向量检索服务（`RAGVectorStore` 类）。
- `app/llm/llm_provider.py`：LLM provider 适配层。
- `app/module/`：Streamlit 模块化 UI（settings、ask_ai、bottom_bar 等）。
...
## 可扩展点与贡献指南
- 添加或支持更多模型/Provider：修改 `app/llm/llm_provider.py` 并在前端设置中暴露所需参数。
- 添加新的文档格式支持（如 DOCX）：扩展 `tools/build_vectordb.py` 的文档解析逻辑。
- 扩展 Agent / Tools：在 `app/agent/` 添加新工具，并在 `app/mcp/` 中提供对应的 MCP 接口。
